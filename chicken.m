% Chicken Scream - Ultimate Edition with Effects and Power Screams
...
score = 0;
jump_thresh = 0.04;
scream_effect_thresh = 0.12; % Super scream effect trigger

%% GUI Setup
f = figure('Name','🐔 Chicken Scream - Ultimate','NumberTitle','off');
...
effect_text = text(10,11,'','FontSize',14,'FontWeight','bold','Color','magenta');
...
while ishandle(f)
    ...
    if vol > jump_thresh && jumps_used < max_jumps
        vy = vol * jump_multiplier;
        jumps_used = jumps_used + 1;
        if vol > scream_effect_thresh
            set(effect_text,'String','💥 POWER SCREAM! 💥');
        else
            set(effect_text,'String','');
        end
    end
